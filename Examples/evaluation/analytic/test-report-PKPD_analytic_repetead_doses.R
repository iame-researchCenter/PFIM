...
modelParameters = list(
  ...
  ModelParameter( name = "C50",  distribution = LogNormal( mu = 0.17, omega = sqrt( 0.7 ) ), fixedMu = TRUE ),
  ModelParameter( name = "ka",   distribution = LogNormal( mu = 1.6, omega = sqrt( 0.1 ) ), fixedOmega = TRUE ),
  ModelParameter( name = "Imax", distribution = LogNormal( mu = 0.73, omega = sqrt( 0.3 ) ), fixedOmega = TRUE  )
)
...
## administration
administrationRespPK = Administration( outcome = "RespPK", tau = 10, dose = c( 150 ) )
...
evaluationPopFIM = Evaluation( name = "PKPD_analytic_repetead_doses_populationFIM",
...
evaluationIndFIM = Evaluation( name = "PKPD_analytic_repetead_doses_individualFIM",
                               modelEquations = modelEquations,
                               modelParameters = modelParameters,
                               modelError = modelError,
                               outcomes = list( "RespPK", "RespPD"),
...
evaluationBayeFIM = Evaluation( name = "PKPD_analytic_repetead_doses_bayesianFIM",
                                modelEquations = modelEquations,
                                modelParameters = modelParameters,
                                modelError = modelError,
                                outcomes = list( "RespPK", "RespPD"),
...
outputFile = paste0("PKPD_analytic_repetead_doses_populationFIM.html")
Report( evaluationPopFIM, saveReportPath, outputFile, plotOptions )

outputFile = paste0("PKPD_analytic_repetead_doses_individualFIM.html")
Report( evaluationIndFIM, saveReportPath, outputFile, plotOptions )

outputFile = paste0("PKPD_analytic_repetead_doses_bayesianFIM.html")
